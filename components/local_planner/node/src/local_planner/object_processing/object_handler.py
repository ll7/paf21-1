...
class ObjectHandler:
    ...
    def _detect_vehicle_in_lane(self, local_route: List[Tuple[float, float]]) -> SpeedObservation:
        ...
        print('Blocked Lane:', blocked_ids)
        if len(blocked_ids) > 0:
            ...
            print(distance, dist(self.vehicle.pos, obj.trajectory[-1]))
    ...
    def plan_route_around_objects(self, local_route: List[Tuple[float, float]]):
        ...
        if closest_object is not None:
            ...
            if dist(self.vehicle.pos, closest_object.trajectory[-1]) < 8:
                return local_route
    ...
    def get_blocked_ids(self, route):
        ...
        for obj_id, obj in objects.items():
            ...
            if len(obj.trajectory) > 3:
                ...
                blocked = ObjectHandler.find_blocked_points(route, obj_positions, threshold=1.8)
                if not blocked:
                    continue
    ...
    def sigmoid_smooth(self, object_speed, object_coordinates, point):
        ...
        if dist(point, self.vehicle.pos) > \
                min([dist(self.vehicle.pos, obj) for obj in object_coordinates]):
